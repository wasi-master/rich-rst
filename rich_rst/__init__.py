...
class RSTVisitor(docutils.nodes.SparseNodeVisitor):
    ...
    def _find_lexer(self, node):
        lexer = (node["classes"][1] if len(node.get("classes")) >= 2 else
                 (node["format"] if node.get("format") else None))
        if lexer is None and self.guess_lexer:
            try:
                ...
                lexer = lexer.aliases[
                    0] if lexer.aliases else self.default_lexer
            if lexer == "text":
    ...
    def visit_paragraph(self, node):
        if hasattr(node, "parent") and isinstance(
                node.parent, docutils.nodes.system_message):
    ...
    def visit_title(self, node):
        style = self.console.get_style("restructuredtext.title",
                                       default="bold")
        self.renderables.append(
            Panel(Align(node.astext(), "center"), box=box.DOUBLE, style=style))
        raise docutils.nodes.SkipChildren()

    def visit_Text(self, node):
        style = self.console.get_style("restructuredtext.text",
    ...
    def visit_admonition(self, node):
        style = self.console.get_style("restructuredtext.admonition",
                                       default="bold white")
        self.renderables.append(
            Panel(
                node.astext().replace("\n", " "),
    ...
    def visit_attention(self, node):
        style = self.console.get_style("restructuredtext.attention",
    ...
    def visit_caution(self, node):
        style = self.console.get_style("restructuredtext.caution",
                                       default="red")
        self.renderables.append(
            Panel(
                node.astext().replace("\n", " "),
    ...
    def visit_danger(self, node):
        style = self.console.get_style("restructuredtext.danger",
                                       default="bold white on red")
        self.renderables.append(
            Panel(
                node.astext().replace("\n", " "),
                title="DANGER: ",
    ...
    def visit_error(self, node):
        style = self.console.get_style("restructuredtext.error",
                                       default="bold red")
        self.renderables.append(
            Panel(
                node.astext().replace("\n", " "),
    ...
    def visit_hint(self, node):
        style = self.console.get_style("restructuredtext.hint",
                                       default="yellow")
        self.renderables.append(
            Panel(
                node.astext().replace("\n", " "),
    ...
    def visit_important(self, node):
        style = self.console.get_style("restructuredtext.important",
                                       default="bold blue")
        self.renderables.append(
            Panel(
                node.astext().replace("\n", " "),
    ...
    def visit_note(self, node):
        style = self.console.get_style("restructuredtext.note",
                                       default="bold white")
        self.renderables.append(
            Panel(
                node.astext().replace("\n", " "),
    ...
    def visit_tip(self, node):
        style = self.console.get_style("restructuredtext.tip",
                                       default="bold green")
        self.renderables.append(
            Panel(
                node.astext().replace("\n", " "),
    ...
    def visit_warning(self, node):
        style = self.console.get_style("restructuredtext.warning",
                                       default="bold yellow")
        self.renderables.append(
            Panel(
                node.astext().replace("\n", " "),
    ...
    def visit_subscript(self, node):
        style = self.console.get_style("restructuredtext.subscript",
                                       default="none")
        if self.renderables and isinstance(self.renderables[-1], Text):
            self.renderables[-1].append(
                Text(node.astext().translate(self.subscript),
                     style=style,
                     end=" "))
            raise docutils.nodes.SkipChildren()
        self.renderables.append(
            Text(node.astext().translate(self.subscript), end="", style=style))
        raise docutils.nodes.SkipChildren()

    def visit_superscript(self, node):
        style = self.console.get_style("restructuredtext.superscript",
                                       default="none")
        if self.renderables and isinstance(self.renderables[-1], Text):
            self.renderables[-1].append(
                Text(node.astext().translate(self.supercript),
                     style=style,
                     end=" "))
            raise docutils.nodes.SkipChildren()
        self.renderables.append(
            Text(node.astext().translate(self.supercript), end="",
                 style=style))
        raise docutils.nodes.SkipChildren()

    def visit_emphasis(self, node):
        style = self.console.get_style("restructuredtext.emphasis",
                                       default="italic")
        if self.renderables and isinstance(self.renderables[-1], Text):
            self.renderables[-1].append(
                Text(node.astext().replace("\n", " "), style=style, end=" "))
    ...
    def visit_strong(self, node):
        style = self.console.get_style("restructuredtext.strong",
                                       default="bold")
        if self.renderables and isinstance(self.renderables[-1], Text):
            self.renderables[-1].append(
                Text(node.astext().replace("\n", " "), style=style, end=" "))
    ...
    def visit_image(self, node):
        ...
        self.renderables.append(
            Text("🌆 ") + Text(
                ...
                style=Style(
                    link=node.get("target", target or "Image")
                    or node.get("uri"),
                    color="#6088ff",
                ),
            ))
    ...
    def visit_bullet_list(self, node):
        ...
        text_style = self.console.get_style(
            "restructuredtext.bullet_list_text", default="none")
        for list_item in node.children:
            nested_list = [
                i for i in list_item.children
                if isinstance(i, docutils.nodes.bullet_list)
            ]
            if nested_list:
                for list_item in list_item.children:
                    self.renderables.append(
                        Text("  ", end="") +
                        Text(" ∘ ", end="", style=marker_style))
                    self.renderables.append(
                        Text(list_item.astext().replace("\n", " "),
                             style=text_style))
    ...
    def visit_enumerated_list(self, node):
        ...
        text_style = self.console.get_style("restructuredtext.enumerated_text",
                                            default="none")
    ...
    def visit_literal(self, node):
        style = self.console.get_style("restructuredtext.inline_codeblock",
                                       default="grey78 on grey7")
        if self.renderables and isinstance(self.renderables[-1], Text):
            self.renderables[-1].append(
                Text(node.astext().replace("\n", " "), style=style, end=" "))
            raise docutils.nodes.SkipChildren()
    ...
    def visit_literal_block(self, node):
        style = self.console.get_style("restructuredtext.literal_block_border",
                                       default="grey58")
        if self.renderables and isinstance(self.renderables[-1], Text):
    ...
    def visit_system_message(self, node):
        self.errors.append(
            Panel(
                ...
                title=
                f"System Message: {node.attributes.get('type', '?')}/{node.attributes.get('level', '?')} ({node.attributes.get('source', '?')}, line {node.attributes.get('line', '?')});",
                border_style={
                    None: "none",
                    "INFO": "bold cyan",
                    "WARNING": "bold yellow",
    ...
    def visit_field(self, node):
        field_name_style = self.console.get_style(
            "restructuredtext.field_name", default="bold")
        field_value_style = self.console.get_style(
            "restructuredtext.field_value", default="none")
        previous_table = None
        if isinstance(self.renderables[-1], Table):
            possible_table = self.renderables[-1]
            if (possible_table.columns[0].header
                    == "Field Name") and (possible_table.columns[1].header
                                          == "Field Value"):
        ...
        if previous_table is False:
            table = Table("Field Name", "Field Value", show_lines=True)
            table.add_row(
                Text(node.children[0].astext(), style=field_name_style),
                Text(node.children[1].astext(), style=field_value_style),
            )
    ...
    def visit_definition_list(self, node):
        term_style = self.console.get_style("restructuredtext.term_style",
                                            default="none")
        classifier_style = self.console.get_style(
            "restructuredtext.classifier_style", default="cyan")
        definitions_style = self.console.get_style(
        ...
        for child in node.children:
            try:
                ...
                if len(child.children) > 2:
                    ...
                    self.renderables.append(
                        Text.from_markup(
                            f"[{classifier_style}]{term.astext()}[/{classifier_style}]"
                        ) + Text("\n    ", end="") + Text(
                            classifier.astext().replace("\n", " "),
                ...
                self.renderables.append(
                    ...
                    Text(definitions.astext().replace("\n", " "),
                         style=definitions_style) + Text("\n", end=""))
        raise docutils.nodes.SkipChildren()

    def visit_option_list(self, node):
        option_string_style = self.console.get_style(
            "restructuredtext.option_string", default="none")
        option_argument_style = self.console.get_style(
        ...
        option_description_style = self.console.get_style(
        ...
        for option_list_item in node.children:
            ...
            for option in option_group.children:
                ...
                option_text += (
                    ...
                    (Text(option_argument.astext(),
                          style=option_argument_style)
                     if option_argument else Text()) + (Text(
                         option_group.child_text_separator,
                         style=option_child_text_separator_style,
                     ) if len(option_group.children) > 1 else ""))
            if description:
                option_text += Text("\n    ")
                option_text += Text(description.astext(),
                                    style=option_description_style)
    ...
    def visit_block_quote(self, node):
        text_style = self.console.get_style("restructuredtext.blockquote_text",
                                            default="white")
        marker_style = self.console.get_style(
            "restructuredtext.blockquote_attribution_marker",
            default="bright_magenta")
        author_style = self.console.get_style(
        ...
        try:
            ...
            self.renderables.append(
                ...
                Text(paragraph.astext(), style=text_style) + Text("\n") +
    ...
    def visit_citation(self, node):
        border_style = self.console.get_style(
    ...
    def visit_citation_reference(self, node):
        style = self.console.get_style("restructuredtext.citation_reference",
                                       default="grey74")
        if self.renderables and isinstance(self.renderables[-1], Text):
    ...
    def visit_header(self, node):
        style = self.console.get_style("restructuredtext.caption",
                                       default="bold")
        self.renderables.insert(
            0,
            Panel(
                Align(node.astext(), "center"),
                title="caption",
                box=box.DOUBLE,
                style=style,
            ),
    ...
    def visit_raw(self, node):
        ...
        if lexer == "html":
            ...
            lexer = (guess_lexer(text).aliases[0]
                     if self.guess_lexer else self.default_lexer)

        self.renderables.append(
            Panel(
                Syntax(text, lexer, theme=self.code_theme),
                border_style=style,
                box=box.SQUARE,
                title=title,
            ))
...
class RestructuredText(JupyterMixin):
    ...
    def __rich_console__(self, console: Console,
        ...
        border_style = console.get_style("restructuredtext.footer_border",
                                         default="grey74")
        footer_text = ""
