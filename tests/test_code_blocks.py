...
def test_raw_html_falls_back_when_tag_stripping_fails(render_text, monkeypatch):
    monkeypatch.setattr(
        'rich_rst.MLStripper.feed', lambda self, html: (_ for _ in ()).throw(ValueError('boom'))
    )
...
"""
    out = render_text(rst)
    assert 'doctest block' in out, "Doctest block must render as a Panel with title 'doctest block'"
