...
def test_bullet_list_in_spanning_grid_table_cell_keeps_markers_and_no_extra_blank_lines(
    render_text,
):
...
def test_flat_table_spanning_cell_preserves_inline_styles(make_visitor):
    ...
    content_line = next(
        (
            line
            for line in table_group.renderables
            if isinstance(line, Text) and 'bold' in line.plain and 'italic' in line.plain
        ),
        None,
    )
...
def test_flat_table_spanning_title_centered_to_rendered_width(make_visitor):
    ...
    title_line = next(
        (
            line
            for line in table_group.renderables
            if isinstance(line, Text) and 'Span Title' in line.plain
        ),
        None,
    )
    top_border = next(
        (
            line
            for line in table_group.renderables
            if isinstance(line, Text) and line.plain.startswith('┏')
        ),
        None,
    )
...
def test_flat_table_multi_header_rows_inner_separator_is_heavy(render_text):
    ...
    assert '━' in inner_sep, 'Intra-header separator must use heavy horizontal ━, got: ' + repr(
        inner_sep
    )
    assert inner_sep.endswith('┫'), 'Intra-header separator must end with heavy ┫, got: ' + repr(
        inner_sep[-4:]
    )
    assert '─' not in inner_sep, 'Intra-header separator must not contain light ─'
...
"""
    ...
    assert '1' in out and '2' in out and '3' in out, 'All simple-table body cells must be visible'
