...
def test_image_alt_text_content_in_plain(render_text):
    assert 'A beautiful photo' in render_text('.. image:: photo.png\n   :alt: A beautiful photo\n')
...
def test_sidebar_body_visible(render_text):
    assert 'Some side content.' in render_text('.. sidebar:: Note\n\n   Some side content.\n')
...
def test_citation_produces_panel_with_citation_title(make_visitor):
    ...
    assert not citation_panels, (
        'Citation panel is created during final RestructuredText rendering, not during visitor walk'
    )
...
def test_footnote_reference_bracket_label_has_grey74_span(make_visitor):
    ...
    assert ref_texts
    grey_spans = [
        s for s in ref_texts[0]._spans if s.style.color and s.style.color.name == 'grey74'
    ]
...
def test_plain_definition_list_uses_term_style_branch():
    ...
    all_plain = ''.join(r.plain for r in visitor.renderables if isinstance(r, Text))
...
def test_definition_list_term_uses_term_style_not_classifier_style():
    ...
    for index in range(term_start, term_start + len('term')):
        ...
        assert style.color.name == 'green', (
            f'Expected green at offset {index}, got {style.color.name}'
        )
...
"""
    ...
    assert 'The definition of the term with classifier' in out, 'Definition body must be visible'
...
"""
    out = render_text(rst)
    assert 'raw latex' in out, "Raw LaTeX must render as a Panel with title 'raw latex'"
...
"""
    out = render_text(rst)
    assert 'raw text' in out, "Raw text must render as a Panel with title 'raw text'"
