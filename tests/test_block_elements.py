...
def test_enumerated_list_marker_style_is_bold_yellow(make_visitor):
    ...
    markers = [
        t for t in texts if t.plain.startswith(' ') and t.plain.strip().rstrip('.').isdigit()
    ]
    assert markers
    assert str(markers[0].style) == 'bold yellow', (
...
"""
    ...
    assert '2: Second note.' in out, (
        "Second numbered footnote should render inline as 'label: body'"
    )
...
"""
    out = render_text(rst)
    assert out.count(' citation ') == 1, (
        'Multiple citations should render in a single citation panel'
    )
    assert 'A: First source.' in out, (
        'First citation should be present in the grouped citation panel'
    )
    assert 'B: Second source.' in out, (
        'Second citation should be present in the grouped citation panel'
    )
...
"""
    ...
    assert '1: First footnote.' in out, (
        "Manual numbered footnote should render inline as 'label: body'"
    )
...
"""
    ...
    assert '1: Auto-numbered footnote.' in out, (
        "Auto-numbered footnote should render inline as 'label: body'"
    )
...
"""
    ...
    assert 'note: A labeled footnote.' in out, (
        "Labeled footnote should render inline as 'label: body'"
    )
...
"""
    out = render_text(rst)
    assert 'Named auto-numbered footnote' in out, (
        'Named auto footnote body must appear in the Footer panel'
    )
