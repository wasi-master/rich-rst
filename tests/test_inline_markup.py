...
def _spans_with(
    text_obj, *, italic=None, bold=None, color_name=None, bgcolor_name=None, underline=None
):
...
def test_abbreviation_in_sentence_has_underline_span(make_visitor):
    t = _get_paragraph_text(
        make_visitor('The :abbreviation:`CSS (Cascading Style Sheets)` standard.')
    )
...
"""
    ...
    code_spans = [
        s for t in texts for s in t._spans if 'grey78' in str(s.style) or 'grey7' in str(s.style)
    ]
