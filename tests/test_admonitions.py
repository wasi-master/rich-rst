...
def test_admonition_body_plain_text_visible(render_text):
    assert 'Important text here.' in render_text('.. note::\n\n   Important text here.\n')
...
def test_admonition_body_inline_code_visible(render_text):
    assert 'caution()' in render_text('.. warning::\n\n   Use ``caution()`` here.\n')


def test_admonition_body_italic_text_visible(render_text):
    assert 'important' in render_text('.. tip::\n\n   This is *important*.\n')
...
def test_compact_versionadded_does_not_inline_onto_preceding_compact_admonition(render_text):
    ...
    assert any('Added in v0.47' in ln and 'Be careful here' not in ln for ln in out.splitlines()), (
        out
    )
...
def test_compact_versionadded_does_not_inline_onto_multiparagraph_admonition_body(render_text):
    ...
    assert any(
        'Added in v0.47' in ln and 'Second paragraph' not in ln for ln in out.splitlines()
    ), out
...
def test_compact_admonition_inside_table_cell(make_visitor):
    ...
    for col in tables[0].columns:
        for cell in col._cells:
            assert not isinstance(cell, _Panel), (
                'compact mode must not produce a Panel inside a table cell'
            )
            for sub in getattr(cell, 'renderables', [cell]):
                assert not isinstance(sub, _Panel), (
                    'compact mode must not produce a Panel inside a cell Group'
                )
...
def test_compact_stacked_admonitions_save_lines():
    ...
    panel_lines = (
        RestructuredText(rst, admonition_style='panel').render_to_string(width=68).splitlines()
    )
    compact_lines = (
        RestructuredText(rst, admonition_style='compact').render_to_string(width=68).splitlines()
    )
...
"""
    out = render_text(rst)
    assert 'Custom Title' in out, "Generic admonition must render with '<title>' panel title"
