...
def _code_spans(t):
    return [
        ...
        for s in t._spans
        if s.style.color
        and s.style.color.name == 'grey78'
        and s.style.bgcolor
        and s.style.bgcolor.name == 'grey7'
    ]
...
def test_func_role_explicit_title_target_not_in_plain(make_visitor):
    t = _get_text(make_visitor, 'Call :func:`custom name <os.path.join>` here.')
    assert 'os.path.join' not in t.plain, 'Target identifier must not appear in the plain text'
...
def test_sphinx_roles_in_mixed_paragraph_all_styled(make_visitor):
    rst = 'This has :func:`function_call` and **bold text** and ``regular code`` together.'
...
def test_program_role_does_not_bold_following_text():
    ...
    console.print(
        RestructuredText(
            ':program:`git` is a distributed version control system.', sphinx_compat=True
        )
    )
